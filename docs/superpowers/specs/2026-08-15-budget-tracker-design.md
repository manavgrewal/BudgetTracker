...
### users
`id, name, username (unique), password_hash (argon2id), role ('admin'|'member'), totp_secret_encrypted (nullable), totp_enabled (bool), is_active (bool), must_change_password (bool, default 0), created_at`

- First-run setup wizard creates the first user as admin.
- Admin creates additional users (name + temporary password); the user **must** set their own password at first login (§6 forced change, v1.5 — implemented). TOTP stays optional and self-service.
...
- **Forced password change (v1.5 — implemented):** `users.must_change_password` is set when an admin creates a user and when an admin resets a password — the two cases where somebody other than the account holder knows the secret. It is **not** set by the setup wizard's own admin (who chose their password) nor by a self-service change from Settings. While the flag is set, every authenticated **page** redirects to `/change-password`; the interstitial takes current password + new password (same `MIN_PASSWORD_LENGTH` rule), clears the flag, and destroys that user's other sessions while keeping the current one. **API routes are deliberately not gated:** the flag's threat model is "an admin knows this password", not "this session is compromised", so it is a UX nudge rather than a session invalidation — an admin who wants the session dead already has Deactivate and Reset password, both of which destroy every session outright. Gating `/api/*` would also break the logout POST and every in-flight fetch for no security gain.
...
- **v1.5** (2026-08-16): **forced password change on first login**, an owner decision that supersedes ruling R29 (which had deferred it as v2 polish). §3 `users` gains `must_change_password` (bool, default 0) — migration `drizzle/0001_add_must_change_password.sql`, the second migration under the hand-maintained regime. §3's "user sets own password at first login" line moves from aspiration to requirement, and §6 gains the forced-change bullet: the flag is raised by admin create and admin reset only, the `/change-password` interstitial clears it and destroys the user's other sessions, and gating is page-level with `/api/*` deliberately left ungated (threat model is "an admin knows this password", not "this session is compromised"). No other section changes.
