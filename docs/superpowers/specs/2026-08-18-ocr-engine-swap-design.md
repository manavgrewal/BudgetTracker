...
**MUST-4.4 (step 6's cap corrected by MUST-4.43 from 4000 to 1600).** `preprocess.ts` takes a filesystem path and returns `{ data: Buffer; width: number; height: number }` holding raw RGB, 3 channels, 8 bits per channel, no alpha. The sharp pipeline is exactly:
...
**MUST-4.43 (Task 3 correction, controller ruling).** `PREPROCESS_MAX_LONG_SIDE_PX` is corrected from `4000` to `1600`. `DET_LIMIT_TYPE = 'min'` (MUST-4.42) supplies no upper bound of its own, so this constant alone stands between a full resolution phone photo and the detector, and 1600 is the bound needed to keep that worst case fast on NAS hardware rather than waiting for section 14's acceptance step A9 to measure it. A long side over 1600 is downscaled to it, aspect preserved, with the same `kernel: 'lanczos3'` resampling MUST-4.4 step 6 already uses, before any other geometric step. The upscale-small half of step 6, raising a small image toward `PREPROCESS_MIN_LONG_SIDE_PX` by up to `PREPROCESS_MAX_UPSCALE`, is unchanged and cannot conflict with the new cap because its floor sits well under it.
...
3. `contours.ts` operates on a bitmap whose long side is at most `PREPROCESS_MAX_LONG_SIDE_PX` (1600), the sharp stage cap from MUST-4.4 step 6 (`DET_LIMIT_TYPE = 'min'` only ever raises the *short* side to `DET_LIMIT_SIDE_LEN` and supplies no upper bound of its own, unlike the `'max'` type this bound originally assumed; see MUST-4.42). A 1600 by 1600 worst case is under three times the bitmap area a 960 by 960 `'max'`-type bound would have produced. MUST-4.43 lowered the cap from an earlier, uncorrected 4000 (over sixteen times that same area) ahead of section 14's acceptance step A9, rather than waiting for it to measure a much larger worst case on real hardware.
...
| `PREPROCESS_MAX_LONG_SIDE_PX` | `1600` | Ours. Bounds the deskew rotate and the crop buffer (Task 3 correction, was `4000`). See MUST-4.43 |
...
- **`detect.test.ts`**: the resize rule rounds to the nearest multiple of 32 and, under `limit_type: 'min'` (MUST-4.7 as corrected by Task 2), raises the shorter side to `DET_LIMIT_SIDE_LEN` when it is below and otherwise leaves both dimensions alone, no upper bound of its own (the input is already capped by `PREPROCESS_MAX_LONG_SIDE_PX`); `scaleX` and `scaleY` round-trip a corner back to within 1 pixel; the first sixteen floats of the tensor for `tensor-4x4.png` match hand-computed values; a returned tensor with mismatched spatial dims throws.
...
| R4 | The pipeline's own JavaScript blocks the event loop, and the app becomes unresponsive while a receipt is read; the tesseract worker process used to prevent this | MUST-4.39 states the change honestly and bounds it three ways: `DET_MAX_BOXES = 200`, a `setImmediate` yield between recognition batches, and a contour bitmap bounded by `PREPROCESS_MAX_LONG_SIDE_PX` (1600) on its long side per MUST-4.43. `session.run` itself is off-thread on libuv. A10 measures the real number on the real NAS |
...
| R11 | The `PREPROCESS_MAX_LONG_SIDE_PX = 1600` cap (MUST-4.43) downscales very large photos and could soften small print on tall receipts, under-delivering on the release's whole premise | Task 2's correction inverted the original 960-crush risk; the only remaining downscale is the 1600 cap. A9 measures it on the owner's own five failing receipts and names raising the cap to 2400 as the tested remedy, a measurement rather than a guess in this document |
...
- **Task 3 correction** (2026-08-18): the controller reviewed Task 2's `DET_LIMIT_TYPE = 'min'` finding and chose not to wait for section 14's acceptance step A9 to learn whether an unbounded detector input is too slow on NAS hardware. `PREPROCESS_MAX_LONG_SIDE_PX` is lowered from `4000` to `1600` (MUST-4.43): a full resolution phone photo is downscaled to `1600` on its long side, aspect preserved, with the same `lanczos3` resampling, before any other geometric step in `preprocess.ts`, and the existing upscale-small floor (`PREPROCESS_MIN_LONG_SIDE_PX`, `PREPROCESS_MAX_UPSCALE`) is unchanged. MUST-4.4, MUST-4.39 and MUST-4.42 are updated to match.
