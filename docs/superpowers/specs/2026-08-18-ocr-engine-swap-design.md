...
1. Downloads the dictionary from the RapidOCR GitHub source tree at tag `v3.9.2`.
2. Refuses a body that does not decode as UTF-8, that contains a CRLF, that contains a blank line other than a single trailing newline, or that has fewer than 90 or more than 200 lines. The English PP-OCR dictionary is a short, one-character-per-line file; those bounds catch an HTML error page, a redirect stub, and the wrong dictionary (the Chinese `ppocr_keys_v1.txt` has thousands of lines).
...
**MUST-4.7** The preprocessed image is resized for detection by this rule, which is PaddleOCR's `DetResizeForTest` with `limit_type = 'max'`:

1. `ratio = DET_LIMIT_SIDE_LEN / max(width, height)`, clamped to at most 1.0 when `DET_LIMIT_TYPE === 'max'`. The detector never upscales; MUST-4.4 step 6 already handled small inputs.
...
**MUST-4.8** The tensor is `float32`, NCHW, shape `[1, 3, resizeH, resizeW]`, channel order **RGB**, built as `value = (pixel / 255 - DET_MEAN[c]) / DET_STD[c]` per channel. The mean and std triples are ImageNet's and are pinned in §4.11. Getting the channel order or the mean and std wrong produces boxes that are subtly wrong rather than absent, which is the hardest class of bug to notice, so §13.2 pins the first sixteen tensor values for a fixed 4 by 4 fixture.
...
3. `contours.ts` operates on a bitmap of at most 960 by 960, which is fixed by `DET_LIMIT_SIDE_LEN` regardless of the input photo's size.
...
| Constant | Value | Provenance |
|---|---|---|
| `DET_LIMIT_SIDE_LEN` | `960` | PaddleOCR `det_limit_side_len` default. See MUST-4.42 |
| `DET_LIMIT_TYPE` | `'max'` | PaddleOCR `det_limit_type` default |
| `DET_SIZE_MULTIPLE` | `32` | DBNet's stride |
| `DET_MEAN` | `[0.485, 0.456, 0.406]` | PaddleOCR det `NormalizeImage`, ImageNet |
| `DET_STD` | `[0.229, 0.224, 0.225]` | Same |
| `DET_SCALE` | `1 / 255` | Same |
...
**MUST-4.42** `DET_LIMIT_SIDE_LEN` is the one constant most likely to need changing after the owner's acceptance run, because a receipt is the tall-narrow worst case for a longest-side cap: a 3000 by 4000 photo is reduced to 720 by 960 and small print goes with it. Its comment names `1536` as the tested alternative and states that raising it is a single-constant change costing roughly 2.5x the detection time. §14 acceptance step A9 is where this gets decided on real data rather than in this document.
