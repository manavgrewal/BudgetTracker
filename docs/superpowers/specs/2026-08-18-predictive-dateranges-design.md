...
**MUST-10.8** The daily-slot evaluators need **no** fingerprint. They run at most once per user per day by construction, and their dedup keys make a second run within the catch-up window a no-op.
...
| R2 | A household's first import of a year of history produces dozens of anomaly alerts at once | Three independent guards, and losing any one of them reopens it: the 60-day household-history floor (MUST-9.10), the 14-day transaction window, and the 5-per-evaluation cap (MUST-9.13). A test drives a 12-month import into a fresh install and asserts zero messages |
