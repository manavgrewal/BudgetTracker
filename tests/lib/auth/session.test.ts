...
describe('cookie policy', () => {
  ...
  it('turns Secure on for a direct https request URL, regardless of headers/env', () => {
    const env = { secretKey: 'x'.repeat(32), trustProxy: false, tz: 'UTC', port: 3000, dataDir: '/data' };
  ...
  it('honours X-Forwarded-Proto only when TRUST_PROXY is on', () => {
    const off = { secretKey: 'x'.repeat(32), trustProxy: false, tz: 'UTC', port: 3000, dataDir: '/data' };
